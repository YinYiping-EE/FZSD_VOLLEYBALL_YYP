...
uint8_t test_length; // 用于测试接收数据长度
        ...
        if (rxconf.RxFrameType == FDCAN_DATA_FRAME && rxconf.IdType == FDCAN_STANDARD_ID)
        {
            for (size_t i = 0; i < idx; ++i)
            {
                if (_hfdcan == can_instance[i]->can_handle && rxconf.Identifier == can_instance[i]->rx_id)
                {
                    ...
                    {
                        /* ===== 修改开始 =====
                         * 原代码通过 (rxconf.DataLength >> 16) & 0xF 获取数据长度，
                         * 但新版 HAL 库中 DLC 直接位于低 4 位，移位导致长度恒为 0，
                         * 使得 memcpy 拷贝 0 字节，rx_buff 全零，电机测量值始终为 0。
                         * DJI 电机反馈帧固定为 8 字节，因此直接赋值固定长度。
                         * ==================== */
                        can_instance[i]->rx_len = 8;                        // 接收长度固定为8
                        memcpy(can_instance[i]->rx_buff, fdcan_rx_buff, 8); // 拷贝全部8字节数据
